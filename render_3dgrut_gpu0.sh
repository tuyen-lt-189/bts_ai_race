...
# Usage:
#
#   GPU_ID=0 bash render_3dgrut_gpu0.sh HCM0181
#   GPU_ID=0 CHECKPOINT_STEP=30000 bash render_3dgrut_gpu0.sh HCM0181
#   GPU_ID=0 CHECKPOINT_STEP=last bash render_3dgrut_gpu0.sh HCM0181
#   GPU_ID=0 ENABLE_EVALUATION=false bash render_3dgrut_gpu0.sh HCM0181
#
set -Eeuo pipefail
IFS=$'\n\t'
...
# GPU selection:
# Default to physical GPU 0. The selected GPU is exposed as logical cuda:0 inside Python.
# Override when needed, for example: GPU_ID=1 bash render_3dgrut_gpu0.sh HCM0181
...
check_runtime() {
    ...
    env CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES="$GPU_ID" PHYSICAL_GPU_ID="$GPU_ID" \
...
expected_gpu = os.environ.get("PHYSICAL_GPU_ID")
if os.environ.get("CUDA_VISIBLE_DEVICES") != expected_gpu:
    raise SystemExit(
        "GPU selection failed: CUDA_VISIBLE_DEVICES does not match PHYSICAL_GPU_ID."
    )
if not torch.cuda.is_available():
    raise SystemExit(f"PyTorch cannot access physical GPU {expected_gpu}.")
...
run_renderer() {
    ...
    echo "PyTorch device       : cuda:0 (physical GPU $GPU_ID)"
    echo "Venv                  : $VENV_DIR"
