...
SCRIPT_VERSION="2026-07-24-v4-safe-cuda-build"
...
# Leave empty for automatic selection. Stable source-build target:
# Python 3.10, 3.11, or 3.12.
PYTHON_BIN="${PYTHON_BIN:-}"
PYTHON_MIN_MINOR="${PYTHON_MIN_MINOR:-10}"
PYTHON_MAX_MINOR="${PYTHON_MAX_MINOR:-12}"
RECREATE_INCOMPATIBLE_VENV="${RECREATE_INCOMPATIBLE_VENV:-1}"
...
python_is_supported() {
    ...
    "$executable" - "$PYTHON_MIN_MINOR" "$PYTHON_MAX_MINOR" <<'PY_VERSION_CHECK' \
        >/dev/null 2>&1
import sys
...
raise SystemExit(
    ...
    and minimum <= sys.version_info.minor <= maximum
    else 1
)
PY_VERSION_CHECK
}
...
select_python_interpreter() {
    ...
    if [[ -n "$PYTHON_BIN" ]]; then
        ...
        [[ -n "$resolved" && -x "$resolved" ]] || die \
            "Requested PYTHON_BIN is unavailable: $PYTHON_BIN"

        if ! python_is_supported "$resolved"; then
            die \
                "PYTHON_BIN=$resolved is Python $(python_version_string "$resolved"), " \
                "but this pipeline requires Python 3.${PYTHON_MIN_MINOR}–3.${PYTHON_MAX_MINOR}."
        fi

        PYTHON_BIN="$(readlink -f "$resolved")"
        echo "[Python] Selected explicit interpreter: $PYTHON_BIN"
        echo "[Python] Version: $(python_version_string "$PYTHON_BIN")"
        return
    fi

    # Prefer system interpreters over the active Conda/base shell. This prevents
    # `python3` from unexpectedly resolving to Python 3.14.
    local -a candidates=(
        /usr/bin/python3.12
    ...
    for candidate in "${candidates[@]}"; do
        ...
        if python_is_supported "$resolved"; then
            PYTHON_BIN="$(readlink -f "$resolved")"
            echo "[Python] Auto-selected interpreter: $PYTHON_BIN"
            echo "[Python] Version: $(python_version_string "$PYTHON_BIN")"
            return
        fi
    done
    ...
    die \
        "No supported Python interpreter was found. Install Python 3.10, " \
        "3.11, or 3.12 and its venv package, then set PYTHON_BIN explicitly."
}

prepare_compatible_venv() {
    if [[ -x "$PY" ]]; then
        if python_is_supported "$PY"; then
            echo "[Python] Existing venv is compatible: $VENV_DIR"
            echo "[Python] Venv version: $(python_version_string "$PY")"
            return
        fi
        ...
        old_version="$(python_version_string "$PY" 2>/dev/null || echo unknown)"

        if [[ "$RECREATE_INCOMPATIBLE_VENV" != "1" ]]; then
            die \
                "Existing venv uses Python $old_version, outside supported " \
                "3.${PYTHON_MIN_MINOR}–3.${PYTHON_MAX_MINOR}: $VENV_DIR"
        fi

        backup_path="${VENV_DIR}.python_${old_version//./_}.backup_$(date +%Y%m%d_%H%M%S)"
        echo "[Python] Existing venv is incompatible: Python $old_version"
        echo "[Python] Moving it to: $backup_path"
        mv "$VENV_DIR" "$backup_path"
    elif [[ -e "$VENV_DIR" ]]; then
        local backup_path=""
        backup_path="${VENV_DIR}.invalid.backup_$(date +%Y%m%d_%H%M%S)"

        if [[ "$RECREATE_INCOMPATIBLE_VENV" != "1" ]]; then
            die "VENV_DIR exists but has no executable Python: $VENV_DIR"
        fi

        echo "[Python] Invalid venv path; moving it to: $backup_path"
        mv "$VENV_DIR" "$backup_path"
    fi

    echo "[Python] Creating venv with $PYTHON_BIN"
    "$PYTHON_BIN" -m venv "$VENV_DIR"

    [[ -x "$PY" ]] || die "Failed to create venv Python: $PY"
    python_is_supported "$PY" || die \
        "New venv has unsupported Python version: $(python_version_string "$PY")"

    echo "[Python] Created venv: $VENV_DIR"
    echo "[Python] Venv version: $(python_version_string "$PY")"
}
...
install_torch_if_missing() {
    ...
    IFS=' ' read -r -a torch_packages <<< "$TORCH_PACKAGES"
...
configure_gsplat_cuda_build() {
    ...
    then
        ...
        echo "WARNING: A minor-version difference may work, but a matching toolkit is safer."
    fi
}
...
setup_python_environment() {
    ...
    if [[ -n "$EXTRA_PIP_REQUIREMENTS" ]]; then
        ...
        IFS=' ' read -r -a extra_packages <<< "$EXTRA_PIP_REQUIREMENTS"
...
if (( $# > 0 )); then
    ...
    SCENES_HCM="$(IFS=' '; echo "${selected_hcm[*]}")"
    SCENES_OBJ="$(IFS=' '; echo "${selected_obj[*]}")"
fi

IFS=' ' read -r -a HCM_ARRAY <<< "$SCENES_HCM"
IFS=' ' read -r -a OBJ_ARRAY <<< "$SCENES_OBJ"
...
echo "[Setup order] GPU → disk → packages → select Python → CUDA → clone gsplat → build/install → pipeline"
check_gpu
disk_guard
install_system_packages
select_python_interpreter
