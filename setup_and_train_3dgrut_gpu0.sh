...
#   9. Supports three built-in ablation presets for photo/raw/PPISP tests.
#
...
#   GPU_ID=0 bash setup_and_train_3dgrut.sh
#
# Select another scene:
#
#   GPU_ID=0 bash setup_and_train_3dgrut.sh HCM0204
#
# RTX 4060 8 GB safer override:
#
#   CAP_MAX=1500000 \
#   NHT_FEATURE_DIM=32 \
#   GPU_ID=0 bash setup_and_train_3dgrut.sh HCM0181
#
# DATASET_SOURCE can still be supplied explicitly for a custom location.
#
# Recommended ablation runs:
#
#   TRAIN_PRESET=ab01_photo_nohorizon \
#   AUTO_RESUME=false \
#   bash setup_and_train_3dgrut.sh HCM0181
#
#   TRAIN_PRESET=ab02_raw_nohorizon \
#   AUTO_RESUME=false \
#   bash setup_and_train_3dgrut.sh HCM0181
#
#   TRAIN_PRESET=ab03_ppisp_nohorizon \
#   AUTO_RESUME=false \
#   bash setup_and_train_3dgrut.sh HCM0181
#
set -Eeuo pipefail
...
# GPU selection:
# Default to physical GPU 0. The selected GPU is exposed as logical cuda:0 to PyTorch.
# Override when needed, for example: GPU_ID=1 bash setup_and_train_3dgrut.sh HCM0181
...
# Training preset:
#   custom                  -> use the variables below directly
#   ab01_photo_nohorizon    -> photo_v4, native distortion, no horizon, no PPISP
#   ab02_raw_nohorizon      -> raw RGB, native distortion, no horizon, no PPISP
#   ab03_ppisp_nohorizon    -> raw RGB, native distortion, no horizon, PPISP
TRAIN_PRESET="${TRAIN_PRESET:-custom}"

APPEARANCE_MODE="${APPEARANCE_MODE:-native_distortion}"
HORIZON_AWARE_LOSS="${HORIZON_AWARE_LOSS:-true}"
...
# Native 3DGUT / NHT controls.
UT_ALPHA="${UT_ALPHA:-0.1}"
PARTICLE_FEATURE_HALF="${PARTICLE_FEATURE_HALF:-true}"
FEATURE_OUTPUT_HALF="${FEATURE_OUTPUT_HALF:-true}"
PARTICLE_KERNEL_MAX_ALPHA="${PARTICLE_KERNEL_MAX_ALPHA:-0.99}"
...
# Regularization.
USE_OPACITY_REG="${USE_OPACITY_REG:-true}"
LAMBDA_OPACITY="${LAMBDA_OPACITY:-0.02}"
USE_SCALE_REG="${USE_SCALE_REG:-true}"
LAMBDA_SCALE="${LAMBDA_SCALE:-0.005}"

# Horizon-aware loss controls.
HORIZON_TOP_REGION_END="${HORIZON_TOP_REGION_END:-0.58}"
HORIZON_SKY_INTERIOR_END="${HORIZON_SKY_INTERIOR_END:-0.24}"
HORIZON_SKY_LUMA_MIN="${HORIZON_SKY_LUMA_MIN:-0.55}"
HORIZON_SKY_EDGE_MAX="${HORIZON_SKY_EDGE_MAX:-0.025}"
HORIZON_EDGE_THRESHOLD="${HORIZON_EDGE_THRESHOLD:-0.040}"
HORIZON_SKY_WEIGHT="${HORIZON_SKY_WEIGHT:-0.25}"
HORIZON_BAND_WEIGHT="${HORIZON_BAND_WEIGHT:-1.25}"
HORIZON_TOP_FOREGROUND_WEIGHT="${HORIZON_TOP_FOREGROUND_WEIGHT:-1.10}"
HORIZON_EDGE_LOSS_WEIGHT="${HORIZON_EDGE_LOSS_WEIGHT:-0.02}"

# PPISP controls used only by APPEARANCE_MODE=ppisp_raw.
PPISP_USE_CONTROLLER="${PPISP_USE_CONTROLLER:-true}"
PPISP_DISTILLATION_STEPS="${PPISP_DISTILLATION_STEPS:-0}"

# Photometric preprocessing controls used by APPEARANCE_MODE=native_distortion.
PHOTOMETRIC_STRENGTH="${PHOTOMETRIC_STRENGTH:-0.55}"
GAIN_MIN="${GAIN_MIN:-0.90}"
GAIN_MAX="${GAIN_MAX:-1.12}"
...
case "$TRAIN_PRESET" in
    custom)
        ;;
    ab01_photo_nohorizon)
        APPEARANCE_MODE="native_distortion"
        HORIZON_AWARE_LOSS="false"
        RUN_NAME="${RUN_NAME:-${SCENE_NAME}_ab01_photo_nohorizon_$((MAX_STEPS / 1000))k_$((CAP_MAX / 1000000))m_fd${NHT_FEATURE_DIM}}"
        ;;
    ab02_raw_nohorizon)
        APPEARANCE_MODE="raw_native"
        HORIZON_AWARE_LOSS="false"
        RUN_NAME="${RUN_NAME:-${SCENE_NAME}_ab02_raw_nohorizon_$((MAX_STEPS / 1000))k_$((CAP_MAX / 1000000))m_fd${NHT_FEATURE_DIM}}"
        ;;
    ab03_ppisp_nohorizon)
        APPEARANCE_MODE="ppisp_raw"
        HORIZON_AWARE_LOSS="false"
        RUN_NAME="${RUN_NAME:-${SCENE_NAME}_ab03_ppisp_nohorizon_$((MAX_STEPS / 1000))k_$((CAP_MAX / 1000000))m_fd${NHT_FEATURE_DIM}}"
        ;;
    *)
        echo "ERROR: Unknown TRAIN_PRESET='$TRAIN_PRESET'." >&2
        echo "Allowed: custom, ab01_photo_nohorizon, ab02_raw_nohorizon, ab03_ppisp_nohorizon" >&2
        exit 1
        ;;
esac
...
echo "Horizon loss     : $HORIZON_AWARE_LOSS"
...
UT_ALPHA = float(os.environ.get("UT_ALPHA", "0.1"))
PARTICLE_FEATURE_HALF = env_bool("PARTICLE_FEATURE_HALF", True)
FEATURE_OUTPUT_HALF = env_bool("FEATURE_OUTPUT_HALF", True)
PARTICLE_KERNEL_MAX_ALPHA = float(
    os.environ.get("PARTICLE_KERNEL_MAX_ALPHA", "0.99")
)
NHT_DECODER_HIDDEN_DIM = int(
    os.environ.get("NHT_DECODER_HIDDEN_DIM", "128")
)
NHT_DECODER_NUM_LAYERS = int(
    os.environ.get("NHT_DECODER_NUM_LAYERS", "3")
)
NHT_DECODER_LR = float(
    os.environ.get("NHT_DECODER_LR", "0.00068")
)
NHT_DECODER_EMA_DECAY = float(
    os.environ.get("NHT_DECODER_EMA_DECAY", "0.95")
)

# Regularization from the official 3DGUT-MCMC-NHT app config.
USE_OPACITY_REG = env_bool("USE_OPACITY_REG", True)
LAMBDA_OPACITY = float(os.environ.get("LAMBDA_OPACITY", "0.02"))
USE_SCALE_REG = env_bool("USE_SCALE_REG", True)
LAMBDA_SCALE = float(os.environ.get("LAMBDA_SCALE", "0.005"))

# Horizon-aware loss.
HORIZON_AWARE_LOSS = env_bool("HORIZON_AWARE_LOSS", True)
HORIZON_REPORT_FILENAME = "_photometric_report.csv"
HORIZON_TOP_REGION_END = float(
    os.environ.get("HORIZON_TOP_REGION_END", "0.58")
)
HORIZON_SKY_INTERIOR_END = float(
    os.environ.get("HORIZON_SKY_INTERIOR_END", "0.24")
)
HORIZON_SKY_LUMA_MIN = float(
    os.environ.get("HORIZON_SKY_LUMA_MIN", "0.55")
)
HORIZON_SKY_EDGE_MAX = float(
    os.environ.get("HORIZON_SKY_EDGE_MAX", "0.025")
)
HORIZON_EDGE_THRESHOLD = float(
    os.environ.get("HORIZON_EDGE_THRESHOLD", "0.040")
)
HORIZON_SKY_WEIGHT = float(
    os.environ.get("HORIZON_SKY_WEIGHT", "0.25")
)
HORIZON_BAND_WEIGHT = float(
    os.environ.get("HORIZON_BAND_WEIGHT", "1.25")
)
HORIZON_TOP_FOREGROUND_WEIGHT = float(
    os.environ.get("HORIZON_TOP_FOREGROUND_WEIGHT", "1.10")
)
HORIZON_EDGE_LOSS_WEIGHT = float(
    os.environ.get("HORIZON_EDGE_LOSS_WEIGHT", "0.02")
)
...
PPISP_USE_CONTROLLER = env_bool("PPISP_USE_CONTROLLER", True)
PPISP_DISTILLATION_STEPS = int(
    os.environ.get("PPISP_DISTILLATION_STEPS", "0")
)
...
GAIN_MIN = float(os.environ.get("GAIN_MIN", "0.90"))
GAIN_MAX = float(os.environ.get("GAIN_MAX", "1.12"))
PHOTOMETRIC_STRENGTH = float(
    os.environ.get("PHOTOMETRIC_STRENGTH", "0.55")
)
JPEG_QUALITY = 98
PNG_COMPRESS_LEVEL = 3
IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff", ".webp"}

_horizon_tag = "horizon" if HORIZON_AWARE_LOSS else "nohorizon"
RUN_NAME = os.environ.get(
    "RUN_NAME",
    (
        f"{SCENE_NAME}_3dgrut_v2_3dgut_mcmc_nht_"
        f"{APPEARANCE_MODE}_{_horizon_tag}_"
        f"{MAX_STEPS // 1000}k_"
...
def main() -> None:
    allowed_modes = {"native_distortion", "raw_native", "ppisp_raw"}
    if APPEARANCE_MODE not in allowed_modes:
        fail(
            "APPEARANCE_MODE must be one of: "
            + ", ".join(sorted(allowed_modes))
        )
    if GPU_ID < 0:
        fail(f"GPU_ID must be a non-negative integer, got GPU_ID={GPU_ID}.")
    ...
    # Build photo_v4 only when it is the training input, or when the
    # horizon-aware loss requires its analysis report.
    if APPEARANCE_MODE == "native_distortion" or HORIZON_AWARE_LOSS:
        photo_summary = prepare_photometric_dataset()
    else:
        source_count = len(list_images(SOURCE_SCENE_ROOT / "images"))
        photo_summary = {
            "source_images": source_count,
            "upper_bright_images": 0,
            "corrected_images": 0,
            "report_path": str(
                (PHOTO_DATASET_ROOT / HORIZON_REPORT_FILENAME).resolve()
            ),
        }
        print(
            "[Photometric] Skipped: raw input selected and horizon loss disabled."
        )

    horizon_report = Path(photo_summary["report_path"]).resolve()
    ...
    if APPEARANCE_MODE == "native_distortion":
        ...
        pp_method = "null"
        load_exif = "false"
    elif APPEARANCE_MODE == "raw_native":
    ...
    print(f"Horizon-aware loss    : {HORIZON_AWARE_LOSS}")
    print(f"Horizon report        : {horizon_report}")
    print(f"NHT decoder           : {NHT_DECODER_HIDDEN_DIM} x {NHT_DECODER_NUM_LAYERS}")
    print(f"NHT decoder LR        : {NHT_DECODER_LR}")
    print(f"NHT decoder EMA       : {NHT_DECODER_EMA_DECAY}")
    ...
    environment["PHYSICAL_GPU_ID"] = str(GPU_ID)
    ...
    manifest = {
        ...
        "horizon_aware_loss": HORIZON_AWARE_LOSS,
        ...
        "nht_decoder_learning_rate": NHT_DECODER_LR,
...
run_training() {
    ...
    export UT_ALPHA
    export PARTICLE_FEATURE_HALF
    export FEATURE_OUTPUT_HALF
    export PARTICLE_KERNEL_MAX_ALPHA
    export NHT_DECODER_HIDDEN_DIM
    export NHT_DECODER_NUM_LAYERS
    export NHT_DECODER_LR
    export NHT_DECODER_EMA_DECAY
    export USE_OPACITY_REG
    export LAMBDA_OPACITY
    export USE_SCALE_REG
    export LAMBDA_SCALE
    export HORIZON_TOP_REGION_END
    export HORIZON_SKY_INTERIOR_END
    export HORIZON_SKY_LUMA_MIN
    export HORIZON_SKY_EDGE_MAX
    export HORIZON_EDGE_THRESHOLD
    export HORIZON_SKY_WEIGHT
    export HORIZON_BAND_WEIGHT
    export HORIZON_TOP_FOREGROUND_WEIGHT
    export HORIZON_EDGE_LOSS_WEIGHT
    export PPISP_USE_CONTROLLER
    export PPISP_DISTILLATION_STEPS
    export PHOTOMETRIC_STRENGTH
    export GAIN_MIN
    export GAIN_MAX
    export DATA_FACTOR
    ...
    echo "Train preset         : $TRAIN_PRESET"
    echo "Appearance mode      : $APPEARANCE_MODE"
    echo "Horizon-aware loss   : $HORIZON_AWARE_LOSS"
    ...
    echo "NHT decoder LR       : $NHT_DECODER_LR"
    echo "NHT decoder EMA      : $NHT_DECODER_EMA_DECAY"
    echo "PPISP controller     : $PPISP_USE_CONTROLLER"
    echo "PPISP distill steps  : $PPISP_DISTILLATION_STEPS"
    echo "Auto resume          : $AUTO_RESUME"
    echo "Save on Ctrl+C       : $SAVE_ON_INTERRUPT"
    echo "TCNN JIT fusion      : $TCNN_JIT_FUSION"
